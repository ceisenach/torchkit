...
class ExponentialFamily2P(BasePolicy):
    ...
    def fisher_information(self,states,batch_approx=False,backend='pytorch'):
        ...
        if backend == 'pytorch':
            ...
            I_12 = I_12.unsqueeze(-1).expand(Df.shape) if I_12 is not None else None
            I_22 = I_22.unsqueeze(-1).expand(Dg.shape)
        elif  backend == 'numpy':
            I_11 = np.tile(np.expand_dims(I_11,axis=2),(1,1,Df.shape[2]))
            I_12 = np.tile(np.expand_dims(I_12,axis=2),(1,1,Df.shape[2])) if I_12 is not None else None
        ...
        I_11_Df = I_11 * Df
        I_12_Df = I_12 * Df if I_12 is not None else None
        I_22_Dg = I_22 * Dg 
    ...
    def fisher_vector_product(self,Df,Dg,I_11_Df,I_12_Df,I_22_Dg,v,backend='pytorch'):
        """
        I_12_Df == None indicates it is zero, but None is used to avoid unnecessary ops
        """
        if backend == 'pytorch':
            Df_T = Df.transpose(1,2)
            Dg_T = Dg.transpose(1,2)
            I_12_Df_T = I_12_Df.transpose(1,2) if I_12_Df is not None else None
            g1,g2 = None,None
            ...
            g2_1 = torch.bmm(I_22_Dg,v2batched.unsqueeze(-1))
            g2_1 = torch.bmm(Dg_T,g2_1)
            if I_12_Df is not None:
                g2_2 = torch.bmm(I_12_Df,v1batched.unsqueeze(-1))
            ...
            I_12_Df_T = np.transpose(I_12_Df,(0,2,1)) if I_12_Df is not None else None
            g1,g2 = None,None
            ...
            # calculate lower partition
            g2_1 = np.matmul(I_22_Dg,v2batched)
            g2_1 = np.matmul(Dg_T,g2_1)
            if I_12_Df is not None:
                g2_2 = np.matmul(I_12_Df,v1batched)
